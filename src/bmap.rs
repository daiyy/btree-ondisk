        ...
        C: NodeCache<P> + Clone,
{
    fn new(bmap: &'b BMap<'a, K, V, P, L, C>) -> Self {
        let root = BtreeNode::<K, V, P>::from_slice_ref(bmap.as_slice()).expect("failed to parse root node");
        let root_node_cap_or_nchild = if root.is_large() {
            root.get_nchild()
        } else {
            let root = DirectNode::<V>::from_slice_ref(bmap.as_slice()).expect("failed to parse root node");
            root.get_capacity()
        };
        ...
        match &self.bmap.inner {
            ...
            NodeType::Btree(btree) => {
                ...
                if let Some(idx) = (self.last_root_node_index..self.root_node_cap_or_nchild).next() {
                    let node = BtreeNode::<K, V, P>::from_slice_ref(self.bmap.as_slice()).expect("failed to parse root node");
